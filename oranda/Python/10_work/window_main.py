...
class GameStart:
    ...
    def render(self, screen):
        ...
        # writeの省略系ですセンター寄せで記入されるのと、テキストはlistで渡してほしい
        def wrt(text_list):
...
class GameScene(Scene):
    ...
    def __init__(self):
        ...
        # マップの一枚一枚の色を変化させている。
        self.color = [[random.randint(50,170)] * stage_height for _ in range(stage_width)]
    ...
    def render(self, screen):
        ...
        # 表示する範囲の計算
        display_width = 7  # 中心のマスを含めて左右3マスずつ表示する
        display_height = 7  # 中心のマスを含めて上下3マスずつ表示する

        player_x, player_y = self.stage.player_x,self.stage.player_y  # プレイヤーの座標を取得

        # 表示範囲の上限と下限を計算
        x_min = max(0, player_x - display_width // 2)
        x_max = min(len(self.map[0]), player_x + display_width // 2 + 1)
        y_min = max(0, player_y - display_height // 2)
        y_max = min(len(self.map), player_y + display_height // 2 + 1)

        # マップを描画
        siz = min(HEIGHT // (y_max - y_min), WIDTH // (x_max - x_min))
        for y in range(y_min, y_max):
            for x in range(x_min, x_max):
                rect_x = siz * (x - x_min)
                rect_y = siz * (y - y_min)
                rect = pygame.Rect(rect_x, rect_y, siz, siz)

                if self.map[y][x] == 0:
                    pygame.draw.rect(screen, (self.color[y][x], 255, self.color[x][y]), rect)
                elif self.map[y][x] == 1:
