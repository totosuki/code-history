...
class GameScene(Scene):
    ...
    def __init__(self):
        ...
        maze_width = 30
        maze_height = 30
        self.maze = maze.Maze(maze_width,maze_height)
        self.maze.create_map()
        self.maze.find_farthest_point()
        self.map = self.maze.map
        self.maze.print_map()
        print(f"高さ{self.maze.goal[0]}幅{self.maze.goal[1]}")
    ...
    def handle_events(self, events):
        for event in events:
            ...
            if self.map[self.maze.goal[0]][self.maze.goal[1]] == 3:
                pass
            else:
                return GameScene()
